...
@app.route('/checkvideo', methods=['POST'])
def check_video():
    input_url = request.json['urls']
    print(input_url)
    outputOfAI = 0
    finalOutput = []
    for url in input_url:
        reference_video_hash = VideoHash(url=url)
        print("NOICE")
        ...
        if repuNoFraud >= repuFraud:
            finalOutput.append("Not Fraud")
        else:
            if outputOfAI == -1:
                finalOutput.append("Fraud")
            else:
                if outputOfAI == 0:
                    finalOutput.append("Not Fraud")
                else:
